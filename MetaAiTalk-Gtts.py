...
# Function to stream the generated response with gTTS (Play the MP3)
def play_gtts_response(response_text):
    try:
        tts = gTTS(text=response_text, lang='hi', slow=False)  # 'en' for English, 'hi' for Hindi, 'ur' for Urdu
        tts.save("response.mp3")
        playsound.playsound("response.mp3", True)
        os.remove("response.mp3")  # Clean up the temporary file after playing
    except Exception as e:
...
while True:
    ...
    if metaai_response:
        ...
        # Use gTTS to generate speech from the assistant's response
        play_gtts_response(metaai_response)
    else:
